...
#NAME: BOHDAN DRAHAN
#TIME: WAY TOO LONG THAN IT COULD BE 
...
class Hand(object):
    ...
    def update(self, word):
        ...
        """
        # TODO
        updated_hand = dict(self.handDict)
        if len(word) == 0:
            print "Your word is empty"

        if not self.containsLetters(word):
            print "Can not update the hand using this word:\"", word,"\""
            return
        if not word.isalpha():
            print "Word \"", word, "\" is not a word. Check it!"
        
        for i in word:
            i = i.lower()
            updated_hand[i] -= 1 
        self.handDict = dict(updated_hand)
    ...
    def containsLetters(self, letters):
        ...
        """
        # TODO
        hand_copy = dict(self.handDict)
        if len(letters) == 0:
            print "Check your letters, it should be not nothing"
            return True
        for i in letters:
            i = i.lower()
            if hand_copy.get(i, 0) < 1:
                return False
            else: hand_copy[i] -= 1
        return True
...
class Player(object):
    ...
    def __cmp__(self, other):
        ...
        """
        # TODO
        if self.getPoints() > other.getPoints(): #might have some bug here. Probably should use isClose function instead of ==
            return 1
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        the wordlist
        returns '.' when no words can be made with the given hand 
        """
        # TODO
        word_exist = False
        for word in wordlist.getList():
            if self.hand.containsLetters(word):
                if not word_exist:
                    ...
                    if getWordScore(word) > getWordScore(best_word):
                        best_word = word
        if word_exist:
            return best_word
        else: return '.'
